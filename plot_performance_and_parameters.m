...
scale_factor_pos = mean(exp(speed_param))*mean(exp(hd_param))*50;
scale_factor_hd = mean(exp(speed_param))*mean(exp(pos_param))*50;
scale_factor_spd = mean(exp(pos_param))*mean(exp(hd_param))*50;
...
box off
set(gca,'fontsize',20)
set(gca,'XLim',[0 7]); set(gca,'XTick',1:15)
set(gca,'XTickLabel',{'PHS','PH','PS','HS',...
    'P','H','S'});
legend('Model performance','Selected model','Baseline')

[~, figName, ~] = fileparts(files(iiFile).name);
